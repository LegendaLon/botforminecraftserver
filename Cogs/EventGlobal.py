...
class Event(commands.Cog):
    ...
    async def on_ready(self): # Когда бот запущен и готов к работе
        ...
        activity = discord.Game(name=status) # Задает статус
        await self.client.change_presence(status=discord.Status.online, activity=activity) # Применяет статус
    ...
    async def on_member_join(self, member): # Когда заходит новый пользователь
        ...
        await channel.send(embed=discord.Embed(description= f'Пользователь ``{member.name}``, присоединился к нам!', color=botconfig.orange)) # Пишет в чат сообщение
        ...
        f'а если нужна версия и IP-Адрес сервера напиши ``{botconfig.PREFIX_COMMAND}{botconfig.ip_private_message_onejoin}``', color=botconfig.orange)) # Пишет новому пользователю в лс
